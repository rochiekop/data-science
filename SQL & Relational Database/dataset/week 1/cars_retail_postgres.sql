...
--THE CHEAPEST PRODUCT BOUGHT BY STORE
select * from products p order by buyprice limit 5;

--PRICE IS 10% LOWER THAN MSRP
select *,p.msrp*0.9 as salesprice,(p.msrp*0.9)*p.quantityinstock as totalsales from products p 

--GET PROFIT IF ALL PRODUCT IS SOLD
select p.productcode,
p.productname,
p.quantityinstock ,
p.buyprice *p.quantityinstock as price_product,
p.msrp*0.9 as salesprice,
(p.msrp*0.9)*p.quantityinstock as totalsales ,
((p.msrp*0.9)*p.quantityinstock) - (p.buyprice *p.quantityinstock) as profit
from products p 
order by ((p.msrp*0.9)*p.quantityinstock) - (p.buyprice *p.quantityinstock) desc
limit 10


--FIND CUSTOMER THAT INCLUDE INCORPORATED COMPANY
select * from customers c where c.customername like '%Inc%';


select distinct c.country from customers c;
select distinct c.city from customers c;
select * from customers c where c.country in ('Japan') or c.city in ('NYC','Brickhaven','San Fransisco')

select * from customers c where c.creditlimit is not null and c.creditlimit > 0 and c.country ='USA'


select * from payments p order by p.amount desc limit 10;