...
SELECT
	SUM(QUANTITYINSTOCK) QUANTITYSTOCK
FROM
	PRODUCTS P;



-- Total category less than 5
SELECT
	p.productline category,
	count(p.productline) total_item
FROM
	products p
GROUP BY
	productline
HAVING
	count(p.productline) < 5
...
SELECT
	...
	p.quantityinstock,
	p.productline,
	avg(p.quantityinstock) OVER(PARTITION BY p.productline) avg_every_category
FROM
	products p
...
-- find correlation price with total quantity sold

SELECT
	productcode,
	priceeach,
	quantityordered,
	DENSE_RANK() over(ORDER BY priceeach desc) price_rank,
	DENSE_RANK() over(ORDER BY quantityordered desc) quantity_rank
FROM
	orderdetails o
...
SELECT
	customernumber,
	paymentdate,
	LEAD(paymentdate,1) over(PARTITION BY customernumber ORDER BY paymentdate asc) next_payment_date,
	amount,
	LEAD(amount,1) over(PARTITION BY customernumber ORDER BY paymentdate asc) next_payment,
	LAG(amount,1) over(PARTITION BY customernumber ORDER BY paymentdate asc) next_payment
FROM
	payments p
ORDER BY
	customernumber 
...
--	GET NTH EXPENSIVE PRICES
	
SELECT
	productname,
	productline ,
	buyprice,
	NTH_VALUE(buyprice, 2) OVER(PARTITION BY productline
ORDER BY
	buyprice DESC RANGE BETWEEN 
UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) second_most_expensive_price,
NTH_VALUE(productname,2)  OVER(PARTITION BY productline
ORDER BY
	buyprice DESC RANGE BETWEEN 
UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) second_most_expensive
FROM
	products p